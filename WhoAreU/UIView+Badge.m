...
- (void)badgeInit
{
    ...
    defaultOriginX = self.frame.size.width - self.badge.frame.size.width/2;
    ...
    self.badgeFont      = [UIFont boldSystemFontOfSize:10];
    self.badgePadding   = 6;
    self.badgeMinSize   = 8;
...
- (void)refreshBadge
{
    ...
    if (!self.badgeValue || [self.badgeValue isEqualToString:kStringNull] || ([self.badgeValue isEqualToString:@"0"] && self.shouldHideBadgeAtZero)) {
...
-(UILabel*) badge {
    ...
    if(!label) {
        label = [[UILabel alloc] initWithFrame:CGRectMake(self.badgeOriginX, self.badgeOriginY, 20, 20)];
        [self setBadge:label];
        [self badgeInit];
