...
#define FTDefaultHeightMargin               16.0f
...
#pragma mark - PopOverMenuConfiguration

@interface PopOverMenuConfiguration ()

@end

@implementation PopOverMenuConfiguration

+ (PopOverMenuConfiguration *)defaultConfiguration
{
    static dispatch_once_t once = 0;
    static PopOverMenuConfiguration *configuration;
    dispatch_once(&once, ^{ configuration = [[PopOverMenuConfiguration alloc] init]; });
...
-(void)setupWithMenuName:(NSString *)menuName menuImage:(id )menuImage
{
    ...
    UIFont *font = [PopOverMenuConfiguration defaultConfiguration].textFont;
    CGFloat w = [PopOverMenuConfiguration defaultConfiguration].menuWidth;
    CGFloat m = [PopOverMenuConfiguration defaultConfiguration].menuTextMargin;
    CGFloat h = [menuName heightWithFont:font maxWidth:w-m*2]+FTDefaultHeightMargin;

    
    CGFloat margin = (h - FTDefaultMenuIconSize)/2.f;
    ...
    CGRect menuNameRect = CGRectMake(menuNameX, 0, configuration.menuWidth - menuNameX - configuration.menuTextMargin, h);
    
    if (!menuImage) {
        menuNameRect = CGRectMake(configuration.menuTextMargin, 0, configuration.menuWidth - configuration.menuTextMargin*2, h);
    }
    else {
    ...
    self.menuNameLabel.numberOfLines = FLT_MAX;
...
{
    return [PopOverMenuConfiguration defaultConfiguration].allowRoundedArrow ? FTDefaultMenuArrowWidth_R : FTDefaultMenuArrowWidth;
}
-(CGFloat)menuArrowHeight
{
    return [PopOverMenuConfiguration defaultConfiguration].allowRoundedArrow ? FTDefaultMenuArrowHeight_R : FTDefaultMenuArrowHeight;
}
...
-(void)drawBackgroundLayerWithAnglePoint:(CGPoint)anglePoint
{
    ...
    BOOL allowRoundedArrow = [PopOverMenuConfiguration defaultConfiguration].allowRoundedArrow;
    ...
    _backgroundLayer.lineWidth = [PopOverMenuConfiguration defaultConfiguration].borderWidth;
    _backgroundLayer.fillColor = [PopOverMenuConfiguration defaultConfiguration].tintColor.CGColor;
    _backgroundLayer.strokeColor = [PopOverMenuConfiguration defaultConfiguration].borderColor.CGColor;
    
    self.backgroundLayer.shadowPath = path.CGPath;
    self.backgroundLayer.shadowColor = [UIColor blackColor].CGColor;
    self.backgroundLayer.shadowOffset = CGSizeMake(2, 2);
    self.backgroundLayer.shadowRadius = 2.0f;
    self.backgroundLayer.shadowOpacity = 0.6f;
...
{
    NSString *menuItem = self.menuStringArray[indexPath.row];
    UIFont *font = [PopOverMenuConfiguration defaultConfiguration].textFont;
    CGFloat w = [PopOverMenuConfiguration defaultConfiguration].menuWidth;
    CGFloat m = [PopOverMenuConfiguration defaultConfiguration].menuTextMargin;
    CGFloat h = [menuItem heightWithFont:font maxWidth:w-m*2];
    
    NSLog(@"STRING:%@ HEIGHT:%.0f WIDTH:%.0f", menuItem, h, w);
    
    return h+FTDefaultHeightMargin;
//    return [PopOverMenuConfiguration defaultConfiguration].menuRowHeight;
}
...
-(UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    if (indexPath.row == _menuStringArray.count-1) {
        ...
        menuCell.separatorInset = UIEdgeInsetsMake(0, [PopOverMenuConfiguration defaultConfiguration].menuTextMargin, 0, [PopOverMenuConfiguration defaultConfiguration].menuTextMargin);
    }
    return menuCell;
}
...
@interface PopOverMenu () <UIGestureRecognizerDelegate>
...
+ (PopOverMenu *)sharedInstance
{
    static dispatch_once_t once = 0;
    static PopOverMenu *shared;
    dispatch_once(&once, ^{ shared = [[PopOverMenu alloc] init]; });
    return shared;
}
...
//    CGFloat menuHeight = [PopOverMenuConfiguration defaultConfiguration].menuRowHeight * self.menuArray.count + self.menuArrowHeight;
    ...
    for (NSString *menuItem in self.menuArray) {
        menuHeight += ([menuItem heightWithFont:font maxWidth:w-m*2]+FTDefaultHeightMargin);
    }
    menuHeight += self.menuArrowHeight;
    ...
    if (menuArrowPoint.x + [PopOverMenuConfiguration defaultConfiguration].menuWidth/2 + FTDefaultMargin > KSCREEN_WIDTH) {
        menuArrowPoint.x = MIN(menuArrowPoint.x - (KSCREEN_WIDTH - [PopOverMenuConfiguration defaultConfiguration].menuWidth - FTDefaultMargin), [PopOverMenuConfiguration defaultConfiguration].menuWidth - self.menuArrowWidth - FTDefaultMargin);
        menuX = KSCREEN_WIDTH - [PopOverMenuConfiguration defaultConfiguration].menuWidth - FTDefaultMargin;
    }
    else if ( menuArrowPoint.x - [PopOverMenuConfiguration defaultConfiguration].menuWidth/2 - FTDefaultMargin < 0){
        menuArrowPoint.x = MAX( FTDefaultMenuCornerRadius + self.menuArrowWidth, menuArrowPoint.x - FTDefaultMargin);
        menuX = FTDefaultMargin;
    }
    else {
        menuArrowPoint.x = [PopOverMenuConfiguration defaultConfiguration].menuWidth/2;
        menuX = senderRect.origin.x + (senderRect.size.width)/2 - [PopOverMenuConfiguration defaultConfiguration].menuWidth/2;
    }
    
    if (arrowDirection == FTPopOverMenuArrowDirectionUp) {
        menuRect = CGRectMake(menuX, (senderRect.origin.y + senderRect.size.height), [PopOverMenuConfiguration defaultConfiguration].menuWidth, menuHeight);
        // if too long and is out of screen
        if (menuRect.origin.y + menuRect.size.height > KSCREEN_HEIGHT) {
            menuRect = CGRectMake(menuX, (senderRect.origin.y + senderRect.size.height), [PopOverMenuConfiguration defaultConfiguration].menuWidth, KSCREEN_HEIGHT - menuRect.origin.y - FTDefaultMargin);
            shouldAutoScroll = YES;
        }
    }
    else {
        menuRect = CGRectMake(menuX, (senderRect.origin.y - menuHeight), [PopOverMenuConfiguration defaultConfiguration].menuWidth, menuHeight);
        // if too long and is out of screen
        if (menuRect.origin.y  < 0) {
            menuRect = CGRectMake(menuX, FTDefaultMargin, [PopOverMenuConfiguration defaultConfiguration].menuWidth, senderRect.origin.y - FTDefaultMargin);
...
-(BOOL)gestureRecognizer:(UIGestureRecognizer *)gestureRecognizer shouldReceiveTouch:(UITouch *)touch
{
    ...
    if ([NSStringFromClass([touch.view class]) isEqualToString:@"UITableViewCellContentView"]) {
        return NO;
    }else if (CGRectContainsPoint(CGRectMake(0, 0, [PopOverMenuConfiguration defaultConfiguration].menuWidth, [PopOverMenuConfiguration defaultConfiguration].menuRowHeight), point)) {
